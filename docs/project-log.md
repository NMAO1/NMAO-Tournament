...
- **round-controller:** added `divide` and `all` steps to `index.ts`.
- **Deploy = CLI on split files only.** An earlier self-contained `index.bundled.ts` was **removed**: the bundler left the entry file's `../_shared` imports (duplicate-declaration error), the dashboard can't resolve `../_shared`, and — most importantly — a flattened `Deno.serve` dropped the `authorize()` gate. `index.ts` keeps an `authorize()` gate (service-role key **or** `nmao.is_staff()`) + CORS; deploy with `supabase functions deploy round-controller --project-ref oxzuavpyoetchwebdejp`. Never overwrite `index.ts` with an ungated flattened version.
- **Schema check:** `supabase/verify_schema.sql` confirms `claim_step`, the `rating_history` unique index, the `division_schemes`/`divisions`/`pods` shapes, `medals`/`medal_shipments`, and `rounds.state` are present (all green in PGlite).
