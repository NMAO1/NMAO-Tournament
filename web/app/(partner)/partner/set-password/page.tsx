...
// Landing for the ambassador's set-password / first-time email link (implicit-flow
// recovery token). Verifies a token_hash in JS (email scanners can't burn it),
// then sends them into the dashboard. Mirrors the school set-password page.
      ...
      <div style={{ width: "100%", maxWidth: 380 }}>
        ...
        {ready === "no-session" && (
          <>
            ...
            <button onClick={() => router.push("/partner/login")} style={{ width: "100%", padding: 13, borderRadius: 11, border: `1px solid ${neutrals.border}`, background: "none", color: neutrals.text, cursor: "pointer", fontWeight: 700 }}>Back to sign in</button>
          </>
        )}

        {ready === "ok" && !done && (
          <>
            <p style={{ color: neutrals.muted, textAlign: "center", fontSize: 13, margin: "0 0 20px" }}>Choose a password for your ambassador account.</p>
