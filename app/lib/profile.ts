...
export type ProfileInfo = {
  ...
  photo: string | null; equippedBadge: string | null;
...
export async function loadProfile(competitorId: string): Promise<ProfileInfo | null> {
  ...
  return {
    ...
    photo: cc.profile_photo_url ?? null, equippedBadge: cc.equipped_badge_code ?? null,
...
export async function setNotifPref(competitorId: string, type: string, enabled: boolean): Promise<void> {
  await supabase.from("notification_prefs").upsert({ competitor_id: competitorId, type, enabled });
}
