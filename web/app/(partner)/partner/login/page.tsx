...
// Ambassador portal sign-in (amb.nmao.us/partner/login). Mirrors the school
// portal: password sign-in + a "email me a link" bootstrap that routes through
// send-partner-setup-link (creates/links the auth account, sends a scanner-safe
// set-password link). Invite-only — accounts are issued by NMAO.
export default function PartnerLogin() {
  ...
  async function reset() {
    ...
    setBusy(true);
    try { await supabase.functions.invoke("send-partner-setup-link", { body: { email: email.trim() } }); } catch { /* ignore */ }
    setBusy(false);
  ...
  return (
    <main style={{ minHeight: "100vh", background: neutrals.bg, color: neutrals.text, display: "flex", alignItems: "center", justifyContent: "center", padding: 20, fontFamily: "Inter, system-ui, sans-serif" }}>
      <div style={{ width: "100%", maxWidth: 380 }}>
        ...
        <h1 style={{ fontFamily: "Georgia, serif", fontSize: 26, textAlign: "center", margin: "0 0 4px" }}>NMAO Ambassador Portal</h1>
        <p style={{ color: neutrals.muted, textAlign: "center", fontSize: 13, margin: "0 0 20px" }}>Sign in to your ambassador dashboard.</p>
        ...
        {msg && <p style={{ color: msg.startsWith("If that") ? status.success : status.danger, fontSize: 13, marginTop: 10, textAlign: "center" }}>{msg}</p>}
        <p style={{ color: neutrals.muted, textAlign: "center", fontSize: 11.5, marginTop: 18, opacity: 0.7 }}>🔒 Invite-only — accounts are issued by NMAO</p>
      </div>
    </main>
  );
}
