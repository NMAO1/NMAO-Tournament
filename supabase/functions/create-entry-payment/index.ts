// =====================================================================
// EDGE FUNCTION: create-entry-payment  (Competitor app — register + pay)
// Registers an entry (unpaid, no video needed yet) for the open round and
// returns a Stripe PaymentIntent client secret for the in-app payment sheet.
// GATED TO THE COMPETITOR / GUARDIAN — the school can never pay from here.
// A webhook (stripe-webhook) flips the entry to 'paid' on success.
//
// AUTH: Verify JWT = ON. Requires env: STRIPE_SECRET_KEY.
// POST { competitor_id, event } -> { ok, clientSecret, amount, entry_id }
// Deploy (editor-safe, no _shared): name = create-entry-payment, Verify JWT ON.
// =====================================================================
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const pi = await stripe.paymentIntents.create({
      ...
      metadata: { entry_id: (entry as any).id, competitor_id: competitorId, round_id: roundId },
    });
    await svc.from("entries").update({ payment_intent_id: pi.id }).eq("id", (entry as any).id);

    return json({ ok: true, clientSecret: pi.client_secret, amount, entry_id: (entry as any).id });
  } catch (e: any) {
    console.error("create-entry-payment error:", e?.message || e);
